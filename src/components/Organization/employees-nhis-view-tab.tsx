...
export const EmployeesNHISViewTab = () => {
  const {
    updateStatus,
    updateEmployee,
    handleSearch,
      ...
      <div className="view-employees__tax__top">
        <div className="view-employees__tax__top-cont">
          <div className="view-employees__tax__top-cont__info">
            ...
            <InfoPopUp>Total amount of NHIS Remittance</InfoPopUp>
          </div>
          <p className="view-employees__tax__top-cont__info-md">
            {!Number.isFinite(data?.data?.totalPayrollNHIS) ? (
              <Skeleton width={150} />
            ) : (
              Util.formatMoneyString(currency)(data?.data?.totalPayrollNHIS)
            )}
          </p>
        </div>
        <div className="view-employees__tax__top-cont">
          <div className="view-employees__tax__top-cont__info">
            <div className="view-employees__tax__top-cont__info-sm">
              <p>Active NHIS Employees</p>
            </div>
            <InfoPopUp>Employees who are on NHIS remittance</InfoPopUp>
          </div>
          <p className="view-employees__tax__top-cont__info-md">
            {!Number.isFinite(data?.data?.nhisEmployees) ? (
              <Skeleton width={150} />
            ) : (
              <>{Util.formatNumber(data?.data?.nhisEmployees)} Employees</>
            )}
          </p>
        </div>
        <div className="view-employees__tax__top-cont">
          <div className="view-employees__tax__top-cont__info">
            <p className="view-employees__tax__top-cont__info-sm">
              Non Active NHIS Employees
            </p>
            <InfoPopUp>Employees who are not on NHIS remittance</InfoPopUp>
          </div>
          <p className="view-employees__tax__top-cont__info-md">
            {!Number.isFinite(data?.data?.nhisEmployees) ? (
              <Skeleton width={150} />
              ...
              <>
                {Util.formatNumber(
                  (data?.meta?.total || 0) - (data?.data?.nhisEmployees || 0),
                )}{' '}
                Employees
              </>
            )}
            ...
            {data?.data?.employees?.map((employee) => {
              return (
                <tr key={employee.id}>
                  ...
                  <td>
                    {Util.formatMoneyString(currency)(employee.nhis?.amount)}
                  </td>
                  ...
                  <td style={{ padding: 0 }}>
                    <InputV2
                      ...
                      defaultValue={employee.nhisId}
                      loading={employeeLoading[`${employee.id}_nhisId`]}
                      disabled={employeeLoading[`${employee.id}_nhisId`]}
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: '22px' }}>
                      ...
                      <Switch
                        ...
                        checked={(employee.nhis?.amount || 0) > 0}
                        loading={
                        ...
                        onChange={updateStatus(employee, 'nhis')}
                      />
                    </div>
                  </td>
                </tr>
