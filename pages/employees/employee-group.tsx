import React from 'react';
import Image from 'next/image';
import Head from 'next/head';
import DashboardLayout from '../../src/layouts/dashboard-layout/DashBoardLayout';
import info_icon from '../../public/svgs/info-icon.svg';
import avatar1 from '../../public/svgs/Avatar1.svg';
import avatar2 from '../../public/svgs/Avatar2.svg';
import avatar3 from '../../public/svgs/Avatar3.svg';
import avatar4 from '../../public/svgs/Avatar4.svg';
import { Button } from '../../src/components/Button/Button';
import SearchInput from '../../public/svgs/search.svg';
import Plus from '../../public/svgs/add-fill.svg';

function EmployeeGroup() {
  return (
    <DashboardLayout>
      <div className="employee-group">
        <Head>
          <title>Employee Group</title>
          <meta name="description" content="Generated by create next app" />
          <link rel="icon" href="/favicon.ico" />
        </Head>
        <div className="employee-group__head">
          <h1 className="employee-group__title">Employee Settings</h1>

          <div className="employee-group__employee-button">
            <Button
              label=" Create Employee group"
              onClick={() => {}}
              className="employee-group__employee-button1"
              type="submit"
            />
            <Button
              label={
                <>
                  <Image src={Plus} alt="plus icon"></Image> {'Add Employee'}
                </>
              }
              onClick={() => {}}
              className="employee-group__submit-btn"
              primary
              type="submit"
            />
          </div>
        </div>

        <div className="employee-group__searchemployee">
          <div className="employee-section__employeeHeader">
            <p>3 Group(s)</p>
          </div>
          <div className="employee-group__searchfield">
            <div className="employee-group__searchinput-box">
              <input
                type="text"
                placeholder="Search by name"
                className="employee-section__search"
              />
              <div className="employee-section__searchImage">
                <Image src={SearchInput} alt="search-image" />
              </div>
            </div>
          </div>
        </div>
        <section className="employee-group__employee-group-cards">
          <div className="employee-group__section">
            <div className="employee-group__content">
              <div className="employee-group__group-title">
                <h5 className="employee-group__header-title">
                  Group Name Here
                </h5>
                <Image
                  src={info_icon}
                  alt="group-employee-group__employee-image"
                  className="view-more-icon"
                />
              </div>
              <p className="employee-group__date-created-name">Date Created</p>
              <p className="employee-group__date-time-created">
                September 16, 2021 | 12:40 PM{' '}
              </p>
              <hr />
              <p className="employee-group__salary-title">Common Salary</p>
              <div className="employee-group__salary-amount-section">
                <p className="employee-group__salary-amount">₦ 200,000</p>
                <div className="employee-group__group-images">
                  <Image
                    src={avatar1}
                    alt="group-employee-group__employee-image"
                    className="employee-group__employee-image"
                  />
                  <Image src={avatar2} alt="" className="employee-group__employee-image" />
                  <Image
                    src={avatar3}
                    alt="group-employee-group__employee-image"
                    className="employee-group__employee-image"
                  />
                  <Image
                    src={avatar4}
                    alt="group-employee-group__employee-image"
                    className="employee-group__employee-image"
                  />
                </div>
              </div>
            </div>
          </div>

          <div className="employee-group__section">
            <div className="employee-group__content">
              <div className="employee-group__group-title">
                <h5 className="employee-group__header-title">
                  Group Name Here
                </h5>
                <Image
                  src={info_icon}
                  alt="group-employee-group__employee-image"
                  className="view-more-icon"
                />
              </div>
              <p className="employee-group__date-created-name">Date Created</p>
              <p className="employee-group__date-time-created">
                September 16, 2021 | 12:40 PM{' '}
              </p>
              <hr />
              <p className="employee-group__salary-title">Common Salary</p>
              <div className="employee-group__salary-amount-section">
                <p className="employee-group__salary-amount">₦ 200,000</p>
                <div className="employee-group__group-images">
                  <Image
                    src={avatar1}
                    alt="employee-image"
                    className="employee-group__employee-image"
                  />
                  <Image
                    src={avatar2}
                    alt="employee-image"
                    className="employee-group__employee-image"
                  />
                  <Image
                    src={avatar3}
                    alt="employee-image"
                    className="employee-group__employee-image"
                  />
                  <Image
                    src={avatar4}
                    alt="employee-image"
                    className="employee-image"
                  />
                </div>
              </div>
            </div>
          </div>
          <div className="employee-group__section">
            <div className="employee-group__content">
              <div className="employee-group__group-title">
                <h5 className="employee-group__header-title">
                  Group Name Here
                </h5>
                <Image
                  src={info_icon}
                  alt="group-employee-group__employee-image"
                  className="view-more-icon"
                />
              </div>
              <p className="employee-group__date-created-name">Date Created</p>
              <p className="employee-group__date-time-created">
                September 16, 2021 | 12:40 PM{' '}
              </p>
              <hr />
              <p className="employee-group__salary-title">Common Salary</p>
              <div className="employee-group__salary-amount-section">
                <p className="employee-group__salary-amount">₦ 200,000</p>
                <div className="employee-group__group-images">
                  <Image
                    src={avatar1}
                    alt="employee-image"
                    className="employee-group__employee-image"
                  />
                  <Image src={avatar2} alt="" className="employee-group__employee-image" />
                  <Image
                    src={avatar3}
                    alt="employee-image"
                    className="employee-group__employee-image"
                  />
                  <Image
                    src={avatar4}
                    alt="employee-image"
                    className="employee-group__employee-image"
                  />
                </div>
              </div>
            </div>
          </div>
        </section>
      </div>
    </DashboardLayout>
  );
}

export default EmployeeGroup;
